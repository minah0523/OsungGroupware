...
<script type="text/javascript">
	...
	$(document).ready(function(){
	    ...
	    $("select#sizePerPage").change(function(){
			...
			frm.method = "GET";
		    frm.action = "<%= ctxPath%>/jieun/note/trash.os";
		    frm.submit();				
	...
	function goNoteSearch() {
		...
		var frm = document.goTrashSelectFrm;
		frm.method = "GET";
	...
	function goTrashDelete() { // 휴지통으로 이동
		...
		if(chkCnt == 0) {
			...
			$.ajax({
		                url : "<%=request.getContextPath() %>/jieun/note/deleteFromTblTrash.os",
		                type: "GET",
		                ...
		                success: function(json){
		                	...
		                	if(result != 1) {
		                        alert("휴지통에서 삭제 실패했습니다!");
		                    }
		                    else{
		                        alert("휴지통에서 삭제 성공했습니다!");
	...
	function goTrashOneDetail(note_no) {
    	...
    	var frm = document.goTrashOneDetailFrm;
	    frm.note_no.value = note_no;
	    
	    frm.method = "GET";
	    frm.action = "<%= ctxPath%>/jieun/note/trashOneDetail.os";
	    frm.submit();		
	}
			...
			<div id="miniHeader">
				<ul id="mimiHeaderGroup" style="padding: 0px; margin-bottom: -15px;">
					<li class="miniHeaderList ">
						<button type="button" id="trash" onclick="goTrashDelete()" data-toggle="tooltip" data-placement="top" title="영구삭제">
                ...
                <table class="table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th  style="width: 110px;">보낸사원</th>
                            <th  style="width: 110px;">받는사원</th>
                    ...
                    <tbody>
                      <c:if test="${not empty noteTrashList}">
	                    	<c:forEach var="trashlist" items="${noteTrashList}" varStatus="status">
		                        <tr>
		                            <td style="width: 40px;"> 
		                            	<input type="checkbox" name="chkBox" id="${trashlist.note_no }">
		                            </td>
		                            <c:if test="${trashlist.note_important == 1}">
		                            ...
		                            <td style="width: 110px;">${trashlist.emp_name} &#60; ${trashlist.fk_emp_no_send} &#62;</td>
		                            <td style="width: 110px;">${trashlist.fk_emp_name} &#60; ${trashlist.fk_emp_no_receive} &#62; </td>
		                            <td style="width: 450px;" onclick="goTrashOneDetail('${trashlist.note_no}')">
		                            	${trashlist.note_title}
		                            </td>
		                            <td style="width: 120px;">${trashlist.note_write_date}</td>
		                            
		                        </tr>                    		
	                    	</c:forEach>
                    	</c:if>
                    	<c:if test="${empty noteTrashList}">
                    		<tr>
