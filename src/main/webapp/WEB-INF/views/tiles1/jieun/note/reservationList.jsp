...
<style type="text/css">
  ...
  #trash:hover, #btnReload:hover, #btnOrder:hover {
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("ul#orderList li").click(function(){
			...
			location.href="<%=request.getContextPath() %>/jieun/note/reservationList.os?orderType="+orderType;
	    ...
	    $("select#sizePerPage").change(function(){
			...
			frm.method = "GET";
		    frm.action = "<%= ctxPath%>/jieun/note/reservationList.os";
		    frm.submit();				
	...
	function goNoteSearch() {
		...
		var frm = document.goReservationListSelectFrm;
		frm.method = "GET";
	...
	function goReservationOneDetail(note_no) {
		
    	var frm = document.goReservationOneDetailFrm;
	    frm.note_no.value = note_no;
	    frm.method = "GET";
	    frm.action = "<%= ctxPath%>/jieun/note/reservationOneDetail.os";
	    frm.submit();		
	}	
	...
	function goNoteTrashDelete() { // 휴지통으로 이동
		...
		if(chkCnt == 0) {
			...
			$.ajax({
		                url : "<%=request.getContextPath() %>/jieun/note/moveToTrashcanReservation.os",
		                type: "GET",
		                ...
		                success: function(json){
		                	...
		                	if(result != 1) {
		                        alert("예약테이블에서 삭제 실패했습니다!");
		                        window.location.reload();
		                    }
		                    else{
		                        alert("예약테이블에서 삭제 성공했습니다!");
		                        window.location.reload();
		                    }
...
<div class="row bg-title"  style="border-bottom: solid .025em gray;">
	...
	<form name="goReservationListSelectFrm" >	
        ...
        <div class="white-box" style="padding-top:5px;">
			<div id="miniHeader">
				<ul id="mimiHeaderGroup" style="padding: 0px; margin-bottom: -15px;">
					...
					<li class="miniHeaderList dropdown " style="margin-left: 825px; ">
            ...
            <div class="table-responsive" style="margin-top: 20px;">
                <table class="table">
                    ...
                    <!-- reservationTempList -->
                    <tbody>
                    	<c:if test="${not empty reservationTempList}">
	                    	<c:forEach var="reservationTemplist" items="${reservationTempList}" varStatus="status">
		                        <tr>
		                            <td style="width: 40px;"> 
		                            	<input type="checkbox" name="chkBox" id="${reservationTemplist.note_no}" >
		                            </td>
		                            <c:if test="${reservationTemplist.note_important == 1}">
									...
									<c:if test="${not empty reservationTemplist.note_orgfilename}">                          
			                            <td style="width: 40px;">
			                            	<span class= "fa fa-paperclip"></span> 	
			                            </td>
		                            </c:if>
		                            <c:if test="${empty reservationTemplist.note_orgfilename}">
		                            	<td style="width: 40px;">
		                            	</td>		                            
		                            </c:if>	
		                            
		                            <td style="width: 70px;">${reservationTemplist.fk_emp_no_receive}</td>
		                            <td style="width: 70px;">${reservationTemplist.fk_emp_name}</td>
		                            <td style="width: 500px;"  onclick="goReservationOneDetail('${reservationTemplist.note_no}')">
		                            	${reservationTemplist.note_title}
		                            </td>
		                            <td style="width: 120px;">${reservationTemplist.note_reservation_date}</td>
		                        </tr>                    		
	                    	</c:forEach>
                    	</c:if>
                    	<c:if test="${empty reservationTempList}">
                    		<tr>
...
<!-- note_no 넘기기 -->
<form name = "goReservationOneDetailFrm">
