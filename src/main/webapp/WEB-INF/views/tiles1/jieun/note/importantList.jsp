...
<style type="text/css">
  ...
  #trash, #btnReload, #btnOrder, #readReceive {
  	 border: 0;
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("ul#orderList li").click(function(){
			...
			location.href="<%=request.getContextPath() %>/jieun/note/importantList.os?orderType="+orderType;
		});		
	    ...
	    $("select#sizePerPage").change(function(){
			...
			frm.method = "GET";
		    frm.action = "<%= ctxPath%>/jieun/note/importantList.os";
		    frm.submit();				
	...
	function goNoteSearch() {
		...
		var frm = document.goImportantListSelectFrm;
		frm.method = "GET";
	...
	//===== 목록에서 제목을 눌렀을때 상세 페이지로 이동하면서 readStatus 컬럼 1로 변경 ===== // 
	function goImportantReceiveOneDetail(note_no) {
    	
		var frm = document.goImportantReceiveOneDetailFrm;
	    frm.note_no.value = note_no;
	    
	    frm.method = "GET";
	    frm.action = "<%= ctxPath%>/jieun/note/importantOneDetail.os";
	    frm.submit();	
	}
...
<div class="row bg-title"  style="border-bottom: solid .025em gray;">
	<div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
	    <h4 class="page-title" style="color:#233B49; padding-left: 25px;">중요 쪽지함</h4>
	</div>
	
	<form name="goImportantListSelectFrm" >
        ...
        <div class="white-box" style="padding-top:5px;">
			<div id="miniHeader">
				<ul id="mimiHeaderGroup" style="padding: 0px; margin-bottom: -15px;">
					...
					<li class="miniHeaderList dropdown " style="margin-left: 740px; ">
            ...
            <div class="table-responsive" style="margin-top: 20px;">
                <table class="table">
                    ...
                    <tbody>
                    	<c:if test="${not empty noteImportantList}">
                    	<c:forEach var="importantlist" items="${noteImportantList}" varStatus="status">
	                        <tr>
	                            <td style="width: 40px;"> 
	                            	<input type="checkbox" name="chkBox" id="${importantlist.note_no}">
	                            </td>
	                            <td style="width: 40px;">
	                            	<span class="fa fa-star" style="color:red;"></span> 
	                            </td>
	                            <c:if test="${importantlist.note_filename != null}" >
		                            <td style="width: 40px;">
		                            	<span class= "fa fa-paperclip"></span> 	
		                            </td>
	                            </c:if>
	                            <c:if test="${importantlist.note_filename == null}" >
		                            ...
		                            <td style="width: 70px;">${importantlist.fk_emp_no_send}</td>
		                            <td style="width: 70px;">${importantlist.emp_name}</td>
		                            <td style="width: 500px;" onclick="goImportantReceiveOneDetail('${importantlist.note_no}')">
		                            	${importantlist.note_title}
		                            </td>
		                            <td style="width: 120px;">${importantlist.note_write_date}</td>
		                            
	                        </tr>                    		
                    	</c:forEach>
                    	</c:if>
                    	<c:if test="${empty noteImportantList}">
                    		<tr>
...
<form name = "goImportantReceiveOneDetailFrm">
