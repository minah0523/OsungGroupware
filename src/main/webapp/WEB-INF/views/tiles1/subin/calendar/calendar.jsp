...
<script src="<%= ctxPath %>/resources/packages/timegrid/main.min.js"></script>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    calendar = new Calendar(calendarEl, {
      ...
      customButtons: { //주말 숨기기 & 보이기 버튼
         ...
         events: function (info, successCallback, failureCallback){
        	 setCheckbox();
        	 $.ajax({
     			url:"<%= request.getContextPath() %>/selectSchList.os",
  ...
  function editMyCal(calendar_no){
	  $("iframe.editIframe").attr('src', '<%= request.getContextPath() %>/editTodo.os');
  }
  ...
  function addSchModalBtn(){
	...
	// db에 넣기
	$.ajax({
		url:"<%= request.getContextPath() %>/addModalSch.os",
		data:{title:title, startday:startday, endday:endday, fk_calendar_no:fk_calendar_no, content:content},
		type:"POST",
		dataType:"JSON",
		success:function(json){
  ...
  function addDetailSchModalBtn() {
	  ...
	  frm.method = "GET";
	  frm.action = "<%= request.getContextPath() %>/goAddDetailSch.os";
	  frm.submit();
	  
  }
  
  // 이벤트를 클릭시 상세보기로 이동하는 버튼
  function viewSch(schedule_no){
	  location.href = "<%= request.getContextPath() %>/editSch.os?schedule_no=" + schedule_no;
  }
...
<!-- Left navbar-header -->
  <div class="navbar-default sidebar cal_sidebar" role="navigation">
      <div class="sidebar-nav navbar-collapse slimscrollsidebar">
      	  ...
      	  <i class="fa fa-calendar fa-fw" aria-hidden="true"></i>캘린더</h2>
      	  <div class="center p-20" style="padding-top: 0px !important;">
              ...
              href="<%= request.getContextPath() %>/goAddDetailSch.os">일정등록</a></span>
          </div>
          <ul class="nav" id="side-menu" style="padding-left: 9%;">
              <li style="padding: 10px 0 0;">
                  ...
                  <div class="add_calendar_box dropdown">
                  	...
                  	<i class="fa fa-plus" style="padding-right: 10px;"></i>내 캘린더 추가</a>
                  ...
                  <div class="add_calendar_box">
                  	...
                  	<i class="fa fa-cog" style="padding-right: 10px;"></i>내 캘린더 관리</a>
                  </div>
	       ...
	       <div class="modal_container">
				 ...
				 <table class="table table-borderless">
			      <tbody>
			        ...
			        <tr>
			          <th>내 캘린더</th>
			          <td><select class="addSchSelect" name="fk_calendar_no"></select></td>
			        </tr>
