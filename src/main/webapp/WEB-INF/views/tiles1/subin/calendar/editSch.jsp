...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		// select 값 변경
		//$("select.addSchSelect").val("${schvo.fk_calendar_no}").change();

		// datepicker에 db에서 받아온 시작, 끝 날짜 넣어주기
		$("input[name=startday]").val("${ schvo.startday }".substring(0, 10));
		$("input[name=endday]").val("${ schvo.endday }".substring(0, 10));
		
		// 일정이 종일일 경우
		if (${bAllday eq 1}) {
			$("input[name=allday]").prop("checked", true);
			$("select.startday_hour").val("${ schvo.startday }".substring(11, 16)).change();
			//$("select.endday_hour").val("${ schvo.endday }".substring(11, 16)).change();	// 종일이면 23:59까지 끊기기 때문에 startday와 endday는 같게 둠
		}else{
			$("input[name=allday]").prop("checked", false);
			$("select.startday_hour").val("${ schvo.startday }".substring(11, 16)).change();
			$("select.endday_hour").val("${ schvo.endday }".substring(11, 16)).change();
		}
			...
			success:function(json){
				...
				if (json.length > 0) {
					$.each(json, function(index, item){
						...
						if("${schvo.fk_calendar_no}" == item.calendar_no){
	...
	function selectAtd(){
		...
		$.ajax({
			url:"<%= request.getContextPath() %>/selectAtd.os",
			data:{schedule_no:"${schvo.schedule_no}"},
			type:"POST",
			dataType:"JSON",
			success:function(json){
				...
				if (json.n != 0) {
					// 참석자 칸 초기화
				     ...
				     $("td.atandee_td").html(atdHtml);
				     $("input.fk_emp_no_receive").val(str);
	...
	function delSchBtn() {
		...
		$.ajax({
			url:"/groupware/delSch.os",
			data:{fk_schedule_no:${schvo.schedule_no}},
			type:"POST",
			dataType:"JSON",
			success:function(json){
	...
	function editSchBtn() {
		...
		// 입력받은 값들 유효성 검사: 끝
		var schedule_no = ${ schvo.schedule_no };

		// db에 넣기
		$.ajax({
			url:"/groupware/doEditSch.os",
			data:{schedule_no:schedule_no, title:title, startday:startday, endday:endday, fk_calendar_no:fk_calendar_no, content:content, fk_emp_no_receive:fk_emp_no_receive},
			type:"POST",
			dataType:"JSON",
			success:function(json){
		...
		<input type="hidden" class="fk_emp_no_receive" />
		 <table class="table table-borderless">
	      <tbody>
	        <tr>
	          <th>일정명</th>
	          <td><input class="form-control title modal_input" maxlength="13" name="title" type="text" value="${ schvo.title }" /></td>
	        </tr>
	        <tr>
	          <th>일시</th>
	          <td>
	        ...
	        <tr>
	          <th>참석자</th>
	          <td class="atandee_td">
				  <a data-toggle="modal" data-target="#findEmpListModal"><i class="fa fa-plus" style="padding: 0 10px;"> 참석자 선택</i></a>
	          </td>
	        </tr>
	        
	        <tr>
	          <th>일정내용</th>
	          <td><textarea class="form-control modal_input" cols="30" rows="8" name="content" style="resize: none;">${ schvo.content }</textarea></td>
	        </tr>
