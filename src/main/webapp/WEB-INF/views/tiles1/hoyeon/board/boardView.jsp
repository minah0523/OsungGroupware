<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 $(document).ready(function(){
	...
	function goAddWrite() {
		...
		$.ajax({
			url:"<%= request.getContextPath()%>/addComments.os",
			data:form_data,
			type:"POST",
			dataType:"JSON",
	...
	function goViewComment(currentShowPageNo) {
		...
		$.ajax({
			url:"<%= request.getContextPath()%>/commentList.os",
	...
	function makeCommentPageBar(currentShowPageNo) {
		...
		$.ajax({
			url:"<%= request.getContextPath()%>/getCommentTotalPage.os", 
			data:{"parentSeq":"${boardvo.seq}",
				  "sizePerPage":"5"},
			type:"GET",
			dataType:"JSON",
 ...
 <div style="padding-left: 5%;">
	...
	<hr>
	  <div class="header">
		<div id="smallHeader">
		 	<span onclick="javascript:location.href='<%= request.getContextPath()%>/boardAdd.os'">새글쓰기</span>
		 	<span onclick="javascript:location.href='<%= request.getContextPath()%>/boardAdd.os?fk_seq=${boardvo.seq}&groupno=${boardvo.groupno}&depthno=${boardvo.depthno}'">답변글쓰기</span>
		 	<span onclick="javascript:location.href='<%=request.getContextPath()%>/boardDel.os?seq=${boardvo.seq}&fk_dept_no=${boardvo.fk_dept_no}'">삭제하기</span>
		 	<span onclick="javascript:location.href='<%=request.getContextPath()%>/boardEdit.os?seq=${boardvo.seq}'">수정하기</span>
		</div>
		
		<div id="smallHeader_right">
		 	<span onclick="javascript:location.href='<%= request.getContextPath()%>/boardList.os?fk_dept_no=${boardvo.fk_dept_no}'">목록</span>
		</div>
	</div>
	 ...
	 <!-- 게시글 내용 시작-------------------------------------------- -->
	 <div>
	 	<h2 style="margin:10px;">${boardvo.title}</h2>
	 </div>
	 ...
	 <div id="boardContentAll">		
	 	...
	 	<table>
	 		<thead style="font-size: 12pt;">
	 			...
	 			<tr>
	 				<th style="font-size:9pt;color:#BDBDBD; font-weight:lighter;">${boardvo.writeDay}</th>
	 			</tr>
