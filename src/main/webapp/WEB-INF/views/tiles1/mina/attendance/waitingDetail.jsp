...
$(document).ready(function(){
	...
	$("button#reject").click(function(){
		...
		frm.method = "POST";
		frm.action = "<%= ctxPath%>/attendance/rejectApproval.os";
		frm.submit();
	});
	...
	$("button#approval").click(function(){
		...
		var frm = document.approvalFrm;
		frm.method = "POST";
		frm.action = "<%= ctxPath%>/attendance/confirmApproval.os";
		frm.submit();
	});
	...
	<div id="titleInBox" style="font-weight: bold; font-size: 19px;">승인하기</div>
	...
	<!-- ------------------------------------------------------------------------------------------------------------------------- -->
		...
		<!-- 해당 결재건의 결재번호를 폼에 넣기 -->
		<input type="hidden" id="attendance_apply_no" name="attendance_apply_no" value="${attvo.attendance_apply_no}"/>
			<!-- 중간결재자가 존재하고, 결재하는 사람이 최종결재자일때만 보이도록 한다 -->
			<c:if test="${(not empty attvo.midapprvo) and (attvo.fk_fin_approver_no eq login_userid)}">
			...
			<c:if test="${attvo.fk_mid_approver_no eq login_userid}">
			...
			<c:if test="${attvo.fk_fin_approver_no eq login_userid}">
