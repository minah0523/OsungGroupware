...
public interface InterScheduleDAO {
	...
	int insertOneAtt(Map<String, String> paraMap);

	// (Modal) 일정 추가하기
	int addModalSch(Map<String, String> paraMap);

	// 아직 만들어지지 않은 일정 시퀀스 번호를 받아옴
	String fk_schedule_no();

	// 일정 상세 추가하기(다수)
	int addDetailSch(Map<String, Object> paraMap);

	// 일정 상세 추가하기 => 일정 참석자 추가(다수의 참가자)
	int insertMultiAtt(Map<String, Object> paraMap);
